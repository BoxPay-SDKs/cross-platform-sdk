...
class NetBankingViewModelTest {
    ...
    private fun bank(title: String) = PaymentMethod(id = title, type = "NetBanking", brand = "NetBanking", title = title)

    @Test
    fun `on init, the bank list is filtered to netbanking methods and sorted`() = runTest {
        repo.getPaymentMethodsResult = ApiResponse.Success(
            listOf(bank("SBI"), bank("HDFC")),
            responseCode = 200,
        )

        val viewModel = NetBankingViewModel(repo, analyticsRepo, fetchStatusRepo)
    ...
    fun `on init, an error is surfaced as UiState_Error`() = runTest {
        repo.getPaymentMethodsResult = ApiResponse.Error(message = "could not load banks")
    ...
    @Test
    fun `onSearch filters the already-loaded bank list without re-hitting the network`() = runTest {
        repo.getPaymentMethodsResult = ApiResponse.Success(listOf(bank("SBI"), bank("HDFC"), bank("HSBC")), responseCode = 200)
        val viewModel = NetBankingViewModel(repo, analyticsRepo, fetchStatusRepo)

        viewModel.onSearch("hs")

        val names = (viewModel.uiState.value as UiState.Success<List<*>>).data
            .map { (it as com.crossplatform.sdk.domain.model.SelectedPaymentMethod).displayName }
        assertEquals(listOf("HSBC"), names)
        assertEquals("hs", viewModel.searchQuery.value)
    }

    @Test
    fun `onSearch with a blank query restores the full list`() = runTest {
        repo.getPaymentMethodsResult = ApiResponse.Success(listOf(bank("SBI"), bank("HDFC")), responseCode = 200)
        val viewModel = NetBankingViewModel(repo, analyticsRepo, fetchStatusRepo)
        viewModel.onSearch("sb")
    ...
    @Test
    fun `postNetBankingRequest with a redirect action stores the url and shows the webview`() = runTest {
        ...
        repo.initiatePaymentResult = ApiResponse.Success(
            PaymentMethodPostResponse(
                ...
                actions = listOf(PaymentActions(type = "redirect", url = "https://bank.example.com/pay")),
                paymentMethod = Method(type = "NetBanking", brand = "NetBanking"),
            ),
            responseCode = 200,
        )
        val viewModel = NetBankingViewModel(repo, analyticsRepo, fetchStatusRepo)
        ...
        assertEquals("https://bank.example.com/pay", viewModel.url.value)
        assertTrue(viewModel.showWebview.value)
        assertEquals(1, analyticsRepo.events.size) // NetBanking only fires PAYMENT_INITIATED, not PAYMENT_CATEGORY_SELECTED
    }

    @Test
    fun `postNetBankingRequest turns off the loading animation on error`() = runTest {
        repo.getPaymentMethodsResult = ApiResponse.Success(emptyList(), responseCode = 200)
        repo.initiatePaymentResult = ApiResponse.Error(message = "bank unavailable")
        val viewModel = NetBankingViewModel(repo, analyticsRepo, fetchStatusRepo)
